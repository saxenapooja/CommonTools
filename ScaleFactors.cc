...
#include "DiTauPOG2012/diTauTriggerSF.C"
...
float ScaleFactors::getDoubleMuTrigWeight(const char* trigName, float muPt1, float muEta1, float muPt2, float muEta2) const
{
  switch(era_)
    {
    case Era2011:
      if(strncmp(trigName, "HLT_DoubleMediumIsoPFTau", 20) == 0)
...
void ScaleFactors::getDoubleTauTrigWeight(const char* trigName, float tauPt, float tauEta) const
{
  TFile file("SFTauTrigger.root", "RECREATE");
  TF1 *SFTauTrigger_Func;
  
  switch(idtype_)
    {
      //case-I
    case oldTauID:
      if(strncmp(trigName, "HLT_DoubleMediumIsoPFTau", 24) == 0)
	SFTauTrigger_Func = new TF1("SFTauTrigger_Func","Eff_HLT_2012ISOPARKED_Tau_OLDID_DATA(tauPt, tauEta)/Eff_HLT_2012ISOPARKED_Tau_OLDID_MC(tauPt, tauEta)",0,500);
      else
	assert(false);
      break;
    ...
    case newTauID:
      if(strncmp(trigName, "HLT_DoubleMediumIsoPFTau", 24) == 0) 
	SFTauTrigger_Func = new TF1("SFTauTrigger_Func","Eff_HLT_2012ISOPARKED_Tau_NEWID_DATA(tauPt, tauEta)/Eff_HLT_2012ISOPARKED_Tau_NEWID_MC(tauPt, tauEta)",0,500);
      else
	assert(false);
      break;
...
// float ScaleFactors::getEleIDIsoWeight(float elePt, float eleEta) const
// {
//   if(mupt >= 10 && mupt < 15)
//     {
//     if (fabs(mueta) < 0.8) return 0.9771;
